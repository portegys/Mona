...
#ifdef NO_TK
  FOR_EACH_GROUP({
    if (G->type & USE_OUTPUT_HIST)
      storeOutputs(G, tick);
    if ((G->type & ERROR_MASKS) && (G->type & USE_TARGET_HIST))
      storeTargets(G, tick);
  });
#else
  FOR_EACH_GROUP({
...
#ifdef NO_TK
  if (alwaysStore || G->type & USE_INPUT_HIST)
#else
...
#ifdef NO_TK
  if (alwaysStore || G->type & USE_OUTPUT_HIST)
#else
...
#ifdef NO_TK
    if ((alwaysStore || G->type & USE_TARGET_HIST) &&
#else
...
#ifdef NO_TK
    if (alwaysStore || G->type & USE_OUT_DERIV_HIST)
#else
...
// Client access.
void     (*clientProc)() = NULL;
int exampleTick = -1;
real **netInputs = NULL;
real **netOutputs = NULL;
  ...
  *correct = TRUE;
  for (; tick < Net->maxTicks && event < E->numEvents; tick++) {
    ...
    if (netInputs != NULL)
    {
      Group G;
      G = Net->group[1];
      offset = OFFSET(G->unit, output);
      for (i = 0; i < G->numUnits; i++) {
        netInputs[i] = (real *) ((char *) (G->unit + i) + offset);
      }
    }
    if (netOutputs != NULL)
    {
      Group G;
      G = Net->group[Net->numGroups - 1];
      offset = OFFSET(G->unit, output);
      for (i = 0; i < G->numUnits; i++) {
        netOutputs[i] = (real *) ((char *) (G->unit + i) + offset);
      }
    }
    if (clientProc != NULL)
    {
...
#ifdef NO_TK
  FOR_EACH_GROUP({
    if ((G->type & USE_TARGET_HIST) && (G->costType & ERROR_MASKS))
      storeTargets(G, Net->currentTick);
  });
#else
  FOR_EACH_GROUP({
