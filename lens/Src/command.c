...
static int C_time(TCL_CMDARGS) {
  ...
  __int64 i64Kernel1, i64Kernel2;
  __int64 i64User1, i64User2;
  DWORD dwKernel1, dwKernel2;
  DWORD dwUser1, dwUser2;
#else
...
#ifdef WIN32
  GetProcessTimes(GetCurrentProcess(), &ftCreation, &ftExit, &ftKernel1, &ftUser1);
...
#ifdef WIN32
  GetProcessTimes(GetCurrentProcess(), &ftCreation, &ftExit, &ftKernel2, &ftUser2);
  time2 = GetTickCount64();
  i64Kernel1 = *((__int64 *) &ftKernel1);
  dwKernel1 = (DWORD) (i64Kernel1 / 10000U);
  i64User1 = *((__int64 *) &ftUser1);
  dwUser1 = (DWORD) (i64User1 / 10000U);
  i64Kernel2 = *((__int64 *) &ftKernel2);
  dwKernel2 = (DWORD) (i64Kernel2 / 10000U);
  i64User2 = *((__int64 *) &ftUser2);
  dwUser2 = (DWORD) (i64User2 / 10000U);
  realticks = (real)(time2 - time1) / (real)1000.0;
  userticks = (real)(dwUser2 - dwUser1) / (real)1000.0;
  systicks = (real)(dwKernel2 - dwKernel1) / (real)1000.0;

  return result("%.3f active  %.3f user  %.3f system  %.3f real",
		(userticks + systicks), userticks, systicks, realticks);
#else  
