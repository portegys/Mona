...
#ifdef _MSC_VER
float NaNfvalue = ((DBL_MAX+DBL_MAX)-(DBL_MAX+DBL_MAX));
double NaNdvalue = ((DBL_MAX+DBL_MAX)-(DBL_MAX+DBL_MAX));
#else
...
flag result(char *fmt, ...) {
  ...
  if (suppressLensOutput) return TCL_OK;  
  va_start(args, fmt);
  vsprintf(Buffer, fmt, args);
...
flag warning(char *fmt, ...) {
  ...
  if (suppressLensOutput) return TCL_ERROR;
  va_start(args, fmt);
#ifdef NO_TK
  sprintf(buf, "Error: ");
  buf += 7;
...
#ifdef WIN32
  return (real) (randProb() * 2 - 1.L) * range + mean;
#else
