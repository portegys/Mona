...
Run a set of Q-Learning meta-maze tests given a maze_maker discovery file

Options: [-contextMaze] <maze_maker maze discovery file>
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 2 || argc > 3)
    {
        fprintf(stderr, "Usage: %s [-contextMaze] <maze_maker maze discovery file>\n", argv[0]);
        return 1;
    }
    if (argc == 2)
    {
    ...
    while (fgets(buf, LINESZ, fp) != NULL)
    {
        ...
        if (strncmp(buf, "numRooms=", 9) == 0)
        {
            ...
            if (contextMaze)
            {
                sprintf(buf, "./maze_tester -Qlearning -trials 200 -cycles 40 -contextMaze -markPath -numRooms %d -numDoors %d -numGoals 1 -contextSizes %d -effectDelayScale %d -metaSeed %d -instanceSeeds ", nr, nd, cs, ed, ms);
            }
            else
            {
                sprintf(buf, "./maze_tester -Qlearning -trials 200 -cycles 40 -markPath -numRooms %d -numDoors %d -numGoals 1 -contextSizes %d -effectDelayScale %d -metaSeed %d -instanceSeeds ", nr, nd, cs, ed, ms);
            }
            for (i = 0; i < k; i++)
            {
                if (i < k - 1)
            ...
            strcat(buf, " | tail -1 | cut -d\"/\" -f1 | cut -d\" \" -f5");
